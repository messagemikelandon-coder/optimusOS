...
- ID: ADR-028
- Date: 2026-07-23
- Status: Accepted — implemented (branch `agent/claude/ai-job-proposals`). Additive `/goal` slice. Draft-only AI; deterministic validation + owner approval mandatory; no autonomous writes; CI uses a deterministic fake (no paid/live call). Bays stays OBSERVE-only.
- Context: `/goal` asks for a **recommendation-only** AI that proposes structured Job Compiler inputs from a diagnostic finding's evidence, where the AI output is **draft-only** and deterministic validation + owner approval are mandatory. Hard constraints: the AI must not invent a diagnosis, VIN facts, prices, availability, labor truth, approval, or payment state; no autonomous estimate/work-order/invoice writes; CI must use deterministic fakes with no paid/live call; provenance (model, prompt version, validation, actor, disposition) must be recorded without secrets; and the design must be provider-neutral.
- Decision: add a provider-neutral `JobInputProposer` ABC (`app/job_proposal.py`) with one `propose(evidence) -> ProposedJobInputs` method, a real `OpenAIJobInputProposer` (structured output; degrades any provider/timeout/parse error to a safe `JobProposalUnavailableError`; **never invoked in CI**), and a `build_job_input_proposer(settings)` factory that tests override to inject a deterministic fake. The proposal **schema is the core safety control**: `ProposedJobInputs`/`ProposedJobService` are `extra='forbid'` and contain **no price / availability / part-number / VIN / approval field at all** — only an evidence summary, assumptions, an overall confidence, recommended services (title + a *bounded labor-hours suggestion* + optional rationale + generic part **categories**), and clarifying questions — so a model structurally cannot emit an invented price/VIN/availability/approval, and a malformed or field-injecting payload is rejected by `validate_proposed_inputs`. The validated proposal is persisted as an **audit record** (`job_input_proposals`, migration `040`) recording model + prompt version + validation status + actor + disposition (proposed/accepted/dismissed) and the validated payload — **no secret**. Four owner/manager-gated routes: `POST /api/diagnostic-findings/{id}/propose-job-inputs` (loads the finding **shop-scoped before** calling the provider, so a cross-shop finding never triggers a call; 503 when no provider is configured), plus list/get/patch-disposition. **The proposal is never applied automatically** — it populates the deterministic compile *form*, which the owner reviews, prices from the real catalog, and compiles through the existing deterministic `compile_job` validation (ADR-025). The frontend "Suggest inputs (AI draft)" button states plainly that it is a draft and the AI sets no prices; all proposal text is `escapeHtml`'d.
- Alternatives considered: letting the AI output priced parts / specific part numbers (rejected — the AI must not invent prices/availability; it emits generic categories the owner maps to real catalog parts that carry the customer price); auto-applying a proposal straight into a compilation/estimate (rejected — draft-only; deterministic validation + owner approval are the gates); coupling to the OpenAI SDK response shape directly in the route (rejected — the provider-neutral ABC + factory keeps the interface swappable and lets CI inject a fake with no live call); a soft (non-`extra='forbid'`) schema that ignores unknown fields (rejected — `extra='forbid'` is what makes field-injection of a price/VIN structurally impossible); making the endpoint work without an owner action (rejected — every route is owner/manager-gated and the finding is shop-scoped before any provider call).
- Consequences: a shop can get an AI *draft* of recommended services + labor-hour estimates + part categories to jump-start the deterministic compiler, with the AI structurally unable to invent prices/VIN/availability/approval and unable to write anything autonomously; every proposal is an auditable, disposition-tracked record. Additive and revert-safe: migration `040` adds one table (downgrade drops it); no existing route/behavior changes; the real provider is dormant until an owner configures a key. Rollback = revert the commit(s) + `alembic downgrade 039_job_compilation_release`.
- Files affected: `app/job_proposal.py`, `app/models.py`, `app/db_models.py`, `alembic/versions/040_job_input_proposals.py`, `app/main.py`, `app/static/index.html`, `app/static/app.js`, `app/static/styles.css`, `tests/test_job_proposals_api.py`, `tests/e2e/test_job_input_proposals_migration.py`, `tests/test_official_ui.py`. Documentation: `docs/context/CURRENT_STATE.md`, `docs/context/KNOWN_ISSUES.md`, `docs/context/SESSION_HANDOFF.md`, and this entry.
- Revisit if: a second provider is added (implement the ABC), the proposal is allowed to pre-fill a compilation for one-click review (still owner-gated + deterministic-validated), or the real provider needs a golden/replay test harness (still no live call in CI).

## ADR-029

- ID: ADR-029
- Date: 2026-07-23
- Status: Accepted — implemented (branch `agent/claude/customer-typeahead`). Additive `/goal` slice. **Frontend-only; no backend or migration change.**
- Context: `/goal` asks for an accessible same-shop customer typeahead on the draft-conversion flow, with bounded search, stable order, hard limits, tenant isolation, and safe states, preserving the atomic conversion + duplicate-VIN protection (ADR-026). The intake convert form previously took a raw numeric "existing customer ID," which is unusable without knowing ids and offers no search.
- Decision: replace the numeric input with an accessible typeahead backed by the **existing** `GET /api/customers?search=` (already owner-gated, shop-scoped via `effective_shop_id`, stably ordered `updated_at desc, id desc`, and bounded by `page_size`) — so no new backend surface or migration is needed. The typeahead is a `role=combobox` input + `role=listbox`/`option` results, debounced (250 ms), keyboard-navigable (arrow/enter/escape), with `aria-expanded`/`aria-activedescendant` and safe empty/loading/error states; it requests `page_size=8` (a hard client cap on top of the server's own bound). It preserves the **hidden** `customer_id` the atomic conversion reads and **clears that id on every keystroke**, so an edited/typed name can never silently keep a stale selected id (which would attach the wrong customer); a stale in-flight response is ignored if the input changed. All customer-supplied text (display name, email, phone) is `escapeHtml`'d before rendering. The conversion backend (`convert_intake_request`) is unchanged, so its atomic customer+vehicle creation and duplicate-VIN 409 are preserved.
- Alternatives considered: a new dedicated typeahead endpoint (rejected — the existing customers search already meets the bounded/shop-scoped/stable-order/hard-limit requirements; a new endpoint would duplicate surface); a native `<datalist>` (rejected — poor control over accessibility, the option subtitle, and the hidden-id/stale-selection semantics); keeping the id set across keystrokes for convenience (rejected — that risks attaching a customer whose name the user edited away; clearing on keystroke is the safe default).
- Consequences: owners can attach an existing same-shop customer by searching by name/email/phone instead of memorizing ids, with tenant isolation and duplicate-VIN protection unchanged and no new backend/DB surface. Additive and revert-safe: revert the single commit (no migration, no backend change). Rollback = revert the commit.
- Files affected: `app/static/index.html`, `app/static/app.js`, `app/static/styles.css`, `tests/test_official_ui.py`. Documentation: `docs/context/CURRENT_STATE.md`, `docs/context/SESSION_HANDOFF.md`, and this entry.
- Revisit if: the customers search endpoint's ordering/limits change (the typeahead relies on them), or the same typeahead is wanted elsewhere (estimate/vehicle customer pickers) — extract it into a shared component.
- **Note for future entries in this file:** the next available number in this file's own sequence is **ADR-030**.
