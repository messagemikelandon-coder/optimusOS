...
**Part A — CI enforcement.** Replace the current GitHub Actions setup (`.github/workflows/ai-coordination.yml` only validates the AI handoff doc — confirmed 2026-07-13, no test/lint/build gate runs in CI today) with a real workflow on PRs and pushes to `main`: Python deps, `ruff format --check`, `ruff check`, `pyright`, `pytest`, `node --check app/static/app.js`, Alembic upgrade-from-clean + downgrade/upgrade round-trip, Docker backend+worker build, `docker compose config -q` (base + staging overlay, no resolved-secret printing), and the existing secret/customer-document exposure scan scripts. Postgres + Redis service containers as needed. Any required gate failing fails the workflow. Document branch-protection recommendations without claiming they're enabled unless actually configured.

**Part B — Synthetic test-account provisioning.** A test-only path to mint synthetic owner/technician accounts for automated and local E2E use — env-flag-gated (e.g. `OPTIMUS_TEST_ACCOUNT_PROVISIONING=true`), hard-refuses in production mode, random credentials, never the real owner's credentials, clearly labeled synthetic records, cascade-cleanup path, no password logging. This directly closes the gap every Phase 5.6 sub-phase's handoff has flagged: *no real authenticated end-to-end browser proof exists for any Phase 5.6 module* — every "live Playwright proof" so far used a client-side `state.auth.authenticated = true` bypass against an unauthenticated session, not a real login.
...
- [ ] Part A — CI enforcement.
- [ ] Part B — Synthetic test-account provisioning.
