...
## Phase 5.6 Sub-phase 0 & 1 — Nav Cleanup + Multi-Role Authorization Foundation (in progress, uncommitted, same branch)
...
- Not done / explicitly out of scope this sub-phase: technician provisioning endpoint, `Technician`/`WorkOrder.assigned_technician_id` tables, any technician-specific route (clock-in/out, own-work-order view). All sub-phase 2 scope per `docs/context/PLANS.md`. Nothing committed, pushed, or deployed — pending owner approval.
