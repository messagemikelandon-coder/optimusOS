...
- Active development phase: **Phase 5.5 is merged to `main` and deployed to staging (see "Post-deploy bug triage" history below and `SESSION_HANDOFF.md`). The Landing Page Redesign (PR #14), the original Overview Dashboard + Approval Queue (PR #15), and Phase 5.6 sub-phases 0-2 — auth foundation + Technicians module (PR #17) — are all merged to `main`. The Overview Dashboard + Approval Queue was then reverted by owner request (see below) after a hand-resolved conflict against the later Phase 5.6 work, since a straight revert of PR #15 no longer applied cleanly. None of this is deployed to staging yet.**
...
## Overview Dashboard & Approval Queue — REVERTED

- The Overview dashboard (metric cards, gauges, Chart.js trend charts, revenue breakdown, rule-based Shop Insights) and the Approval Queue nav view, originally shipped in commit `97e8b9d` and merged to `main` via PR #15, were reverted at the owner's request (PR #16) after later Phase 5.6 work (auth foundation, Technicians module) had already built on top of the same files. See `docs/context/KNOWN_ISSUES.md`'s Historical Resolved Issues for the reasoning and the manual conflict-resolution detail — the straight `git revert` conflicted in 8 files since it was generated against a stale base, so this revert was hand-resolved on top of current `main` to preserve everything from the auth foundation and Technicians sub-phases.
- Removed: `app/dashboard_store.py`, the `GET /api/dashboard/summary` route, the Approval Queue nav item + `#view-approval-queue` view + its JS (`loadApprovalQueue`, `renderApprovalQueueList`, `refreshApprovalQueueBadge`, `initializeApprovalQueue`), the vendored Chart.js assets, and the dashboard-specific frontend JS (`loadDashboardSummary`, dashboard rendering functions). The `#view-dashboard` panel reverted to its pre-`97e8b9d` "Shop intelligence online" hero (Direct command quick-chat box + Live systems health panel, both preserved in their original location — they predate the dashboard work).
- Preserved: the Technicians module's nav visibility/routing logic in `app/static/app.js` (`applyRoleNavVisibility`, technician-branch post-login/reload routing, the work-orders technician-options loading) and its interaction with the now-reverted dashboard routing — the technician branch is unaffected since it never depended on dashboard code; the owner branch's `navigate("dashboard")` now lands on the restored old hero instead of the removed Overview page.
- The "Talk to Optimus" nav-item removal (Phase 5.6 sub-phase 0) was **kept**, not restored — that was a separate, unrelated change from the same era, not part of what's being reverted.
