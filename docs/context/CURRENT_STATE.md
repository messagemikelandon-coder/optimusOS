...
- Active development phase: **Phase 5.5 — the owner-approved 4-feature slice (retire transient estimate endpoint, customer history, notifications tab + badge, Square sandbox invoices) is IMPLEMENTED with all gates green (200 tests, ruff, pyright, alembic 009↔010 rehearsal, live proofs) but UNCOMMITTED, pending independent review and owner commit approval. Phase 5 staging droplet redeploy is still pending (see Owner action items in `SESSION_HANDOFF.md`).**
- Current branch: `agent/claude/notify-history-square` (off `ops/staging` `15481c6`, which is pushed to `origin/ops/staging`); all Phase 5.5 work is uncommitted in this worktree.
- Git working state: Phase 2 merged via PR #6 (`8be1dba`); Phase 3 via PR #7 (`423192b`); Phase 4 via PR #8 (`c920891`); Phase 5 staging prep via PR #9 (`36b861b`) and PR #10 (`a1b84ff`, merged on GitHub between sessions — remote `main` already contains the invoice-button fix `1139499`). `ops/staging` carries two further commits: `7d665c8` (optimusctl `COMPOSE_OVERRIDE_FILE` support) and `15481c6` (ruff-format drift cleanup). Old remote feature branches were deleted on GitHub.
