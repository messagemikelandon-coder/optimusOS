...
| `shop_id` on every business table | **Nullable column added as of Phase 3 slice 2** (`alembic/versions/023_shop_id_nullable_columns.py`): all 30 business tables that use `owner_user_id` now also have a nullable `shop_id` FK (`ondelete="CASCADE"`) to `shops.id` plus an index, generated mechanically (AST script cross-referencing every `owner_user_id` column, same methodology as Phase 1's threadpool fix) so the 30 tables and their column definitions can't drift from each other. Deliberately still inert: no backfill, no NOT NULL constraint, no store-module query changes — every existing query still scopes by `owner_user_id`/`effective_owner_id` exactly as before. Verified live against real Postgres (upgrade from a clean DB and from 022, downgrade back to 022 confirmed all 30 columns/constraints/indexes removed cleanly, re-upgrade). Backfilling real `shop_id` values, constraining to NOT NULL, and cutting store-module queries over to `shop_id` are each their own later, separate slices per `/goal`'s own staged migration plan. | Partial (schema only; not yet populated or read) | Phase 3 slice 3+ |
