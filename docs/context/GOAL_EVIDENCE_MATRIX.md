...
|---|---|---|---|
| Async routes not blocking the event loop | **Confirmed false, with one caveat found by independent review.** Nearly every route in `app/main.py` is `async def` calling a synchronous, blocking store function directly (e.g. `create_appointment`, `create_customer`) with no offload. Exactly 3 pre-existing exceptions already correctly use `asyncio.to_thread`: the `/api/chat` route and the two Square push/refresh routes — all genuine external-I/O-bound calls, not the ~130 DB-bound CRUD routes this finding is about. The blanket "every route" wording in an earlier version of this row overclaimed universality; corrected here. | Empirically confirmed 2026-07-17: two "concurrent" HTTP requests with a deliberate 1s delay inserted took ~2s total, not ~1s. `docs/context/KNOWN_ISSUES.md`'s "Request-level concurrency" entry. | Absent — this session's own finding, not yet fixed |
