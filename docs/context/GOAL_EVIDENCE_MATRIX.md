...
| `shop_id` on every business table | **Nullable column added (slice 2) and populated (slice 3)**. Slice 2 (`alembic/versions/023_shop_id_nullable_columns.py`): all 30 business tables that use `owner_user_id` also have a nullable `shop_id` FK (`ondelete="CASCADE"`) to `shops.id` plus an index, generated mechanically (AST script cross-referencing every `owner_user_id` column) so the 30 tables and their column definitions can't drift from each other. Slice 3 (`alembic/versions/024_backfill_shop_id.py`): backfills the real `shop_id` for every existing row via `owner_user_id` → that owner's active `ShopMembership` → `shop_id`; idempotent (`WHERE shop_id IS NULL`), and any row with no matching active owner membership is left NULL and reported via a migration-time warning rather than guessed. Still no NOT NULL constraint and no store-module query changes in either slice — every existing query still scopes by `owner_user_id`/`effective_owner_id` exactly as before. Verified live against real Postgres for both slices (clean-DB upgrade, downgrade, re-upgrade; slice 3 additionally verified the orphan-row case, idempotent re-run, and that downgrade clears data but keeps the column) plus permanent e2e coverage (`tests/e2e/test_shop_tenant_migration_backfill.py`, `tests/e2e/test_shop_id_backfill.py`). Constraining to NOT NULL and cutting store-module queries over to `shop_id` are each their own later, separate slices per `/goal`'s own staged migration plan. | Partial (populated; not yet constrained or read by any query) | Phase 3 slice 4+ |
