...
|---|---|---|---|
| `Shop`/`ShopSettings`/`ShopMembership`/`ShopInvitation`/`ShopRole`/`ShopStatus` models | **Complete.** Slice 1 (`022_shop_tenant_model`) created the models/tables and real-data backfill. The final Phase 3 slice (`028_membership_tenant_boundary`) makes Manager a first-class `UserAccount.role`, broadens the single-active-membership constraint to every role, backfills any real technician login created after migration 022, and makes technician login provisioning create its membership in the same transaction. | Complete | Phase 3 final slice |
| `shop_id` on every business table | **Complete end to end.** Slices 2-6 added, backfilled, auto-populated, orphan-checked, and constrained `shop_id` across all 30 business tables. The final slice cuts every production business-store read/query from `owner_user_id` to `effective_shop_id(db, auth)`, while retaining `owner_user_id` only as compatibility/audit data. Every authenticated request also validates exactly one active membership whose role matches the account. A committed AST regression fails if any production store reintroduces an `owner_user_id` authorization comparison. | Complete | Phase 3 final slice |
| Cross-shop isolation tests | Existing cross-owner tests still cover every module. New `tests/test_membership_tenant_boundary.py` deliberately points a Manager's legacy `shop_owner_id` at a different shop and proves both direct store and real HTTP customer reads/writes follow the active membership's `shop_id`; it also proves the database rejects a second active membership and statically audits every production store query. `tests/e2e/test_membership_tenant_migration.py` proves migration 028 repairs the historical migration-022 inactive-account state, backfills late real technicians, excludes synthetic leftovers without clearing their compatibility pointer, canonicalizes rows only to an active owner account, accepts Manager rows, downgrades safely when no Manager exists, and upgrades again. | Complete | Phase 3 final slice |
...
| Email verification | **Complete and merged via PR #60 (`2e99c7b`).** Migration `027_email_verification` adds `email_verified_at` plus a hash-only, expiring, single-use, revocable token ledger with one active token per account. Signup requests a token through a non-sending adapter; public confirmation and authenticated resend are separately Redis-rate-limited; raw tokens, message bodies, and recipient addresses stay out of logs. Email-bearing unverified accounts are restricted to verification/session-recovery routes, while legacy accounts with no email remain compatible. The SPA includes a public `/verify-email` flow, including cookie-free verification. | Complete | Phase 5 slice 1 |
| Password reset/change | **Complete locally.** Authenticated password change verifies the current password, rejects reuse, preserves the current session, and revokes other sessions/reset grants. Public reset request/confirm uses generic responses, separately rate-limited flows, verified-email eligibility, hash-only expiring single-use tokens, and revokes every active session. | Complete; publication pending | Phase 5 slice 2 |
| Session listing/revocation | **Complete locally.** Authenticated users can list bounded-metadata active sessions, revoke one, revoke all others, or revoke the current session; status changes and password reset revoke sessions transactionally. | Complete; publication pending | Phase 5 slice 2 |
| Login-event history (user-facing) | **Complete locally.** Persistent bounded login events record success/failure/lockout, and the System bay shows the signed-in user's recent activity without exposing another account's history. | Complete; publication pending | Phase 5 slice 2 |
| Account lockout/throttling | **Complete locally.** Existing IP throttling remains; persistent per-account failure counters and defensive cooldown now apply generic failures while allowing correct credentials to clear the cooldown, avoiding username lockout denial of service. | Complete; publication pending | Phase 5 slice 2 |
| Owner/manager/technician invitations | **Complete locally.** Hash-only expiring single-use/revocable invitations support all roles for Owners and technician-only delegation for Managers. Acceptance revalidates current inviter authority, creates one matching membership, serializes against direct Technician provisioning, links a unique same-shop unassigned Technician profile when available, and fails closed on races/ambiguity. | Complete; publication pending | Phase 5 slice 2 |
| MFA-ready architecture | **Schema/API readiness complete locally.** Provider-neutral factor identifiers/status/timestamps and a security summary are present; no raw shared-secret column exists. Actual provider enrollment/challenge UX remains intentionally out of scope. | Architecture complete; provider integration deferred | Phase 5 slice 2 |
