...
| `shop_id` on every business table | Still absent — every business table uses `owner_user_id` (a `UserAccount.id`), confirmed across all 22 migrations. Deliberately deferred per `/goal`'s own staged migration plan (nullable `shop_id` → backfill → constrain → cutover → cleanup, each its own reviewable slice) — slice 1 only creates the Shop entity and attaches existing accounts to it via `ShopMembership`, it does not touch any business table. | Absent | Phase 3 slice 2+ |
