...
| `shop_id` on every business table | **Nullable column added (slice 2), backfilled for existing rows (slice 3), and now auto-populated on every new row (slice 4)**. Slice 2 (`alembic/versions/023_shop_id_nullable_columns.py`): nullable `shop_id` FK + index on all 30 business tables, generated mechanically so the tables and column definitions can't drift from each other. Slice 3 (`alembic/versions/024_backfill_shop_id.py`): one-time backfill of the real `shop_id` for every existing row via `owner_user_id` → that owner's active `ShopMembership` → `shop_id`; idempotent, any unmatched row left NULL and reported via a migration-time warning rather than guessed. Slice 4 (`app/shop_store.py::resolve_shop_id`/`resolve_shop_id_for_owner`, all 30 store modules' CREATE functions): every new row now gets a real `shop_id` at insert time — either resolved from the authenticated request, or mirrored from an already-resolved parent row for child/event/log tables. Found and fixed in the same slice: `provision_synthetic_owner` (`app/test_support_store.py`) never created a `ShopMembership` for synthetic test-support owners, so every row created under one (used throughout the e2e suite) would have silently kept `shop_id = NULL` forever — fixed and proven via both a reverted-and-confirmed regression test and a dedicated e2e test hitting the real HTTP API. Independent + security review of slice 4's initial PR found and fixed 4 more "mirror the parent" call sites the mechanical pass missed (`WorkOrderStatusEvent`, `WorkOrderNote`, plus two `EstimateRevision`/`EstimateApprovalRequest` sites) and all 7 `record_notification` call sites (redundant queries, not bugs), and found a real test-coverage gap (only `Customer` was proven end-to-end) — closed with `tests/test_shop_id_populated_on_create.py`, a fast sqlite test walking one owner through all 30 tables via the real route functions, confirmed via revert-and-recheck to catch a regression. Still no NOT NULL constraint and no *read*/query-scoping changes in any of these three slices — every existing query still scopes by `owner_user_id`/`effective_owner_id` exactly as before. Verified live against real Postgres for slices 2-3 (clean-DB upgrade/downgrade/re-upgrade, orphan-row and idempotency cases) and end-to-end for slice 4 (manual DB-level proof plus two e2e/fast tests against the real app/API and all 30 tables); full gate suite green throughout (402 fast tests, 14 e2e tests). Constraining to NOT NULL and cutting store-module *read* queries over to `shop_id` are each their own later, separate slices — the NOT NULL slice specifically should add an explicit, CI-checkable zero-orphans check (per slice 3's own review note) rather than relying on migration console output. | Partial (populated for new and existing rows; not yet constrained or read by any query) | Phase 3 slice 5+ |
