...
| Self-service shop onboarding | **Backend complete, no frontend form yet.** `POST /api/signup` (public, own rate limiter) creates a brand-new `Shop`/`ShopSettings`/`ShopMembership`/owner `UserAccount` in one transaction (`app/shop_store.py::signup_shop_owner`) and logs the new owner in immediately. Added a nullable `email`/`email_normalized` column pair to `user_accounts` (migration `026_user_account_email.py`), unique platform-wide only where present. Real validation: username/email uniqueness, email format, password strength. Found and fixed in the same slice: `NonBlank = Field(min_length=8, ...)` — used for login and technician-provision passwords too — never actually enforced 8 characters anywhere due to a Pydantic constraint-merging conflict; fixed with a dedicated `Password` type, applied to all 3 affected fields. Verified: migration rehearsed live against real Postgres (upgrade/downgrade); cross-shop isolation between two signed-up shops proven directly; a real e2e test hits the live API end to end. Full gate suite green (410 fast tests, 17 e2e tests). No self-service password reset, email verification, or "resumable setup guidance" checklist yet — those are `/goal` Phase 5/6/12's own separate requirements. No frontend signup form built yet (backend-first, matching this session's established slice-boundary convention) — a follow-up. | Partial (backend done; no frontend, no resumability/checklist yet) | Phase 4 (frontend + guidance remain) |
