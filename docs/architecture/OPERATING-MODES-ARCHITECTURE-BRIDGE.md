...
Status (updated 2026-07-22): **design implemented as non-enforcing slices.** This document began as design/docs only; the capability-resolution service, the OBSERVE-only Bays pilot, owner/manager settings-based operating-mode management, capability-shaped navigation, and owner-only non-blocking post-signup onboarding have since shipped to `main` — see the dated amendments §12a–§12d. The design body (§1–§11) is otherwise preserved. **No capability enforcement has shipped** (Bays stays OBSERVE-only; an AST safeguard blocks any route from referencing `CapabilityGateMode.ENFORCE`), and hidden data is never deleted. The most recent slice — owner-only, non-blocking post-signup onboarding — merged as PR #81 (merge commit `7050bb8d630e4e64e5fca5ee51511a17e54f4507`, feature head `22cfada9c2782910c8aeafc6f8a5fd212df3cdc2`, migration head `035_operating_mode_confirmed_at`). See [ADR-022](adr/ADR-022-operating-mode-tier-separation.md) for the accepted decision this document elaborates.
