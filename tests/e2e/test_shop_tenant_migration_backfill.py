...
def test_backfill_creates_one_shop_per_real_owner_and_excludes_synthetic_accounts(
    ...
    with engine.begin() as connection:
        ...
        # backfill, /goal Phase 7; migration 034's operating-mode backfill and
        # migration 035's onboarding-confirmation backfill, ADR-022)
        # legitimately extend this list.
        assert [row.event_type for row in events] == [
            "shop_backfilled_from_existing_owner",
            "subscription_grandfathered",
            "operating_mode_backfilled",
            "operating_mode_confirmation_backfilled",
        ]
    engine.dispose()
