...
def apply_mode_transition(
    ...
    *,
    ...
    if shop is None:
        raise ModeTransitionError("This shop does not exist.")
    # Row-lock the shop for the check-then-set, matching subscription_store's
    # `_require_subscription(..., for_update=True)` pattern -- so two
    # concurrent applies serialize and the second sees the first's new mode.
