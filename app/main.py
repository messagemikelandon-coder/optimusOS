...
# Phase 2C Step 7: the /api/schedule-blocks routes live in
# app/api/routers/schedule_blocks.py. Same identical-contract mount + handler
# re-export pattern; the five handlers are re-exported because tests call
# them via app.main.
app.include_router(schedule_blocks_router.router)
create_schedule_block_record = schedule_blocks_router.create_schedule_block_record
list_schedule_block_records = schedule_blocks_router.list_schedule_block_records
...
# The /api/schedule-blocks routes were extracted verbatim to
# app/api/routers/schedule_blocks.py (Phase 2C Step 7). They are mounted via
# app.include_router(schedule_blocks_router.router) near app assembly, and
# the handler functions are re-exported there so existing callers
# (main.create_schedule_block_record, etc., across test_scheduling_api /
# test_shop_id_populated_on_create) keep working.
